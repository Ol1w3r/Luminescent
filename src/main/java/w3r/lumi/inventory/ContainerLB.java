...
public class ContainerLB extends Container {
	...
	public ContainerLB(InventoryPlayer playerInv, TileEntityLightBox te) {
		this.te = te;
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index >= SLOTS_INVENTORY && index <= SLOTS_HOTBAR + 9) {
				if (TileEntityFurnace.isItemFuel(stack)) {
					int s = TileEntityLightBox.SLOT_FUEL;
